...
const serverURLlocalhost = "http://localhost:8000"
...
function Login() {
  ...
  const setErrorMessage = (message)=>{
    ...
    setMessage(message)
    settimeoutID(setTimeout(function(){setMessage("")},7000));
}
  ...
  const handleToast = async (random_text)=>{
    console.log(random_text)
    showToast(random_text)
  }
  const login = async ()=>{
    console.log("email :",email);
    console.log("password :",password);
    try{
        const response = await fetch(serverURLlocalhost+'/login',{
      // const response = await fetch('https://myapp-server-egd0dadxbraxf4cj.israelcentral-01.azurewebsites.net/login',{
        ...
        const text = data.random_text
        !text?handleToast("there is no random text"):handleToast(text)
        // handleToast(text)
        }
        else{
